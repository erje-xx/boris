...
Kalinikos and Slavin (K \& S) have solved Maxwell's equation in the magnetostatic limit for a medium described by the linearised Landau-Lifshitz equation, subject to electromagnetic and ``exchange" boundary conditions. Using perturbation theory, K \& S have obtained an explicit expression for ``spin-waves" in the medium. Namely,
...
We consider the physical quantities $\omega_{H}$, $\omega_{M}$, $\alpha$, and $L$ to be constant. Then, for ``in-plane" oriented magnetization, i.e. for fixed $\theta = \frac{\pi}{2}$, equation (\ref{dispersion_main}) may be regarded as an equation in polar coordinates $(k_{\zeta},\phi)$ for $\omega_{n}$. We may therefore use elementary techniques to derive the group velocity $\vec{v}_{g} = \vec{\nabla} \omega_{n} = \frac{\partial \omega_{n}}{\partial k_{\zeta}} \hat{k}_{\zeta} + \frac{1}{k_{\zeta}} \frac{\partial \omega_{n}}{\partial \phi} \hat{\phi}$. We begin with $\frac{\partial \omega_{n}}{\partial k_{\zeta}}$.
