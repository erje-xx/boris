...
def vdwdk(kzeta, phi, mp):
	...
	dFndk_3 = 2*((-1)**n)*exp(-kzeta*L)/kzeta
	...
	E = wm*sin(theta)*sin(theta)*sin(phi)*sin(phi)
	dFnndk_1 = dPnndk
	dFnndk_2 = -dPnndk*sin(theta)*sin(theta)*(1 + cos(phi)*cos(phi))
	...
	omega = sqrt(Q*(Q + wm*Fnn))

	dbetadk = dQdk*(2*Q + wm*Fnn) + wm*Q*dFnndk
	M = wm*sin(theta)*sin(theta)*Pnn*(1 - Pnn)/Q
	
	dwdkzeta = (1/(2*omega))*dbetadk
	# note change due to henning's check, compare with previous results
	dwdphi = (1/(2*omega))*Q*wm*sin(2*phi)*(Pnn*sin(theta)*sin(theta) + M)

	dwdkz = dwdkzeta*(cos(phi)) - (1/kzeta)*dwdphi*sin(phi)
	dwdky = dwdkzeta*(sin(phi)) + (1/kzeta)*dwdphi*cos(phi)
