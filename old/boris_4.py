...
def boris_calculates():
	...
	if abs(group_velocity_test) != 0:
		...
		group_velocity(WORKING_DIRECTORY + 'group_veloctiy.dat', Ms, H, d, gamma, alpha, phi_selected, L, theta, k_max, trans_n)
...
def group_velocity(filename, Ms, H, d, gamma, alpha, phi_selected, L, theta, k_max, n):
	...
	for i in range(0,phi_resolution + 1):
		...
		for l in range(1,int(k_max + 1)):
			...
			dispdiv_out.write(str(kz) + '\t' + str(ky) + '\t' + str(dwdk_temp[0]) + '\t' + str(dwdk_temp[1]) + '\n')
...
def slowness_surface_equal(filename, Ms, H, d, gamma, alpha, theta, n, omega_fixed, k_max, L):
	...
	while True:
		...
		# If we've been in both quadrants, time to go south of the border
		#if ky > 0 and kz > 0:
			#if quadrant_1 == 0:
				#print('Been to quadrant I!')
			#if quadrant_1 == 1 and quadrant_2 == 1:
				#print('Looped it!')
				#kz_init = -kz_init
			#quadrant_1 = 1
		#if ky < 0 and kz > 0:
			#if quadrant_2 == 0:
				#print('Been to quadrant II!')
			#quadrant_2 = 1
	#
		#if ky > 0 and kz < 0:
			#if quadrant_3 == 0:
				#print('Been to quadrant III!')
			#if quadrant_3 == 1 and quadrant_4 == 1:
				#print('Looped the south!')
				#break
			#quadrant_3 = 1
		#if ky < 0 and kz < 0:
			#if quadrant_4 == 0:
				#print('Been to quadrant IV!')
			#quadrant_4 = 1
