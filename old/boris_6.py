...
def dwdk_test(kzeta, phi, Ms, H, d, gamma, alpha, L, theta, k_max, n):
	...
	A = 0.0
	Fo = (2/(kzeta*d))*(1 - math.exp(-kzeta*d))
	#Foo = 1 - math.cos(phi)*math.cos(phi) - 3*Fo/2 + A*Fo*Fo/4
	Foo = 0

	omega_n = math.sqrt(wh*(wh + wm*Foo))

	dFodk = -2/(d*kzeta**2) + 2*math.exp(-d*kzeta)/(d*kzeta**2) + 2*math.exp(-d*kzeta)/kzeta
	dFoodk = -3*A*dFodk/2 + Fo*A*dFodk/2
	dbetadk = wm*wh*dFoodk
