...
def boris_calculates():
	...
	if abs(slowness_surface_equal_test) != 0:
		slowness_surface_equal_out = open(WORKING_DIRECTORY + 'slowness_surface_equal.dat', 'w')
		...
		slowness_surface_equal(WORKING_DIRECTORY + 'slowness_surface_equal.dat', Ms, H, d, gamma, alpha, theta, trans_n, freq, k_max, L)
...
def slowness_surface(filename, Ms, H, d, gamma, alpha, theta, n, omega_fixed, k_max, L):
...
def slowness_surface_equal(filename, Ms, H, d, gamma, alpha, theta, n, omega_fixed, k_max, L):
	slowness_surface_equal_out = open(filename, 'a')
...
def dwdk_new(kzeta, phi, Ms, H, d, gamma, alpha, L, theta, k_max, n):
	pi = math.pi
	wh = gamma*H
	wm = gamma*Ms
	...
	A = math.sin(theta)*math.sin(theta)*(1 + math.cos(phi)*math.cos(phi))
	C = wm*math.sin(theta)*math.sin(theta)*math.sin(phi)*math.sin(phi)
	M = wm*math.sin(theta)*math.sin(theta)*Pnn*(1 - Pnn)/kern

	Fnn_1 = Pnn
	Fnn_2 = math.sin(theta)*math.sin(theta)
	Fnn_3 = (-1)*math.sin(theta)*math.sin(theta)*Pnn*(1 + math.cos(phi)*math.cos(phi))
	Fnn_4 = math.sin(theta)*math.sin(theta)*wm*Pnn*(1 - Pnn)*math.sin(phi)*math.sin(phi)/kern
	
	Fnn = Fnn_1 + Fnn_2 + Fnn_3 + Fnn_4
	
	omega_n = math.sqrt((kern)*(kern + wm*Fnn))

	dFndk = -2/(kzeta**2*d) + 2*((-1)**n)*math.exp(-kzeta*d)*(1/(d*kzeta**2) + 1/kzeta)

	dPnndk_1 = (2*kzeta)/(kn**2) - (2*kzeta**3)/(kn**4)
	dPnndk_2 = (Fn*B*4*kzeta**5)/(kn**6) - (Fn*B*4*kzeta*3)/(kn**4)
	dPnndk_3 = (-B*dFndk*kzeta**4)/(kn**4)
	dPnndk = dPnndk_1 + dPnndk_2 + dPnndk_3

	dFnndk = (1 + C/kern - (2*Pnn*C)/kern - A)*dPnndk + ((C*Pnn**2)/(kern**2) - (C*Pnn)/(kern**2))*dkerndk

	dw2dk = dkerndk*(2*kern + wm*Fnn) + wm*kern*dFnndk

	# Have to multiply by 2pi, since we work with the gyromagnetic ratio
	# in Hz/Oe...i.e. we work with normal gamma divided by 2pi
	dwdkzeta = 2*pi*(1/(2*omega_n))*dw2dk
	dwdphi = 2*pi*(1/(2*omega_n))*kern*wm*math.cos(2*phi)*(Pnn*math.sin(theta)*math.sin(theta) + M)
