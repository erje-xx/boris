...
def boris_calculates():
	...
	for line in conf_file:
		...
		if form_line[0].strip() == "SATURATION_MAGNETIZATION":
			...
			group_velocity = float(form_line[1].strip())
		else:
			continue
	...
	if abs(slowness_surface_test) != 0:
		slowness_surface_equal(Ms, H, d, gamma, alpha, theta, trans_n, freq, k_max, L)

	if abs(group_velocity_test) != 0:
		group_velocity(Ms, H, d, gamma, alpha, phi_selected, L, theta, k_max, trans_n)
...
def group_velocity(Ms, H, d, gamma, alpha, phi_selected, L, theta, k_max, n):
	...
	for i in range(0,181):
		...
		for l in range(1,int(k_max + 1)):
			...
			dispdiv_out.write(str(ky) + '\t' + str(kz) + '\t' + str(dwdk_temp[0]) + '\t' + str(dwdk_temp[1]) + '\n')
...
def slowness_surface_equal(Ms, H, d, gamma, alpha, theta, n, omega_fixed, k_max, L):
	#WORKING_DIRECTORY = '/home/erje/boris/'
	WORKING_DIRECTORY = os.getcwd() + '/'
	slowness_surface_out = open(WORKING_DIRECTORY + 'slowness_surface_equal.dat', 'w')

	pi = math.pi

	epsilon = omega_fixed/10**3
